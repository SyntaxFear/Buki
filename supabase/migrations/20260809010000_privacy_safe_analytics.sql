...
create table public.analytics_events (
  ...
  source text check (source is null or source ~ '^[a-z0-9_]{1,80}$'),
  ...
  plan text check (plan is null or plan in ('monthly', 'yearly', 'lifetime')),
  result text check (result is null or result in ('success', 'not_found', 'failed', 'cancelled')),
  export_kind text check (export_kind is null or export_kind in (
    'png',
    'jpg',
    'share_card',
    'pdf',
